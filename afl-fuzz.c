...
struct queue_entry {
  ...
  region_t *regions; /* Regions keeping information of message(s) sent to the
                        server under test */
  u32 region_count;  /* Total number of regions in this seed */
  u32 index;         /* Index of this queue entry in the whole queue */
  u32 generating_state_id; /* ID of the start at which the new seed was
                              generated */
  u8 is_initial_seed;      /* Is this an initial seed */
  u32 unique_state_count;  /* Unique number of states traversed by this queue
                              entry */
};
...
char **use_argv; /* argument to run the target program. In vanilla AFL, this is
...
u32 *response_bytes =
    NULL; // an array keeping accumulated response buffer size
...
u32 state_cycles = 0;
u32 messages_sent = 0;
EXP_ST u8 session_virgin_bits[MAP_SIZE]; /* Regions yet untouched while the SUT
                                            is still running */
EXP_ST u8 *cleanup_script; /* script to clean up the environment of the SUT --
                              make fuzzing more deterministic */
EXP_ST u8 *netns_name;     /* network namespace name to run server in */
char **was_fuzzed_map =
    NULL; /* A 2D array keeping state-specific was_fuzzed information */
...
region_t *(*extract_requests)(unsigned char *buf, unsigned int buf_size,
...
void expand_was_fuzzed_map(u32 new_states, u32 new_qentries) {
  int i, j;
  // Realloc the memory
  was_fuzzed_map = (char **)ck_realloc(
      was_fuzzed_map, (fuzzed_map_states + new_states) * sizeof(char *));
  for (i = 0; i < fuzzed_map_states + new_states; i++)
    was_fuzzed_map[i] = (char *)ck_realloc(
        was_fuzzed_map[i], (fuzzed_map_qentries + new_qentries) * sizeof(char));
  ...
  // Update total number of states (rows) and total number of queue entries
  // (columns) in the was_fuzzed_map
  fuzzed_map_states += new_states;
  fuzzed_map_qentries += new_qentries;
}

/* Get unique state count, given a state sequence */
u32 get_unique_state_count(unsigned int *state_sequence,
...
/* Check if a state sequence is interesting (e.g., new state is discovered).
 * Loop is taken into account */
u8 is_state_sequence_interesting(unsigned int *state_sequence,
                                 unsigned int state_count) {
  ...
  for (i = 0; i < state_count; i++) {
    if ((i >= 2) && (state_sequence[i] == state_sequence[i - 1]) &&
      ...
      continue;
    count++;
    trimmed_state_sequence =
        (u32 *)realloc(trimmed_state_sequence, count * sizeof(unsigned int));
...
/* Update the annotations of regions (i.e., state sequence received from the
 * server) */
void update_region_annotations(struct queue_entry *q) {
  ...
  for (i = 0; i < messages_sent; i++) {
    if ((response_bytes[i] == 0) ||
        (i > 0 && (response_bytes[i] - response_bytes[i - 1] == 0))) {
      q->regions[i].state_sequence = NULL;
      q->regions[i].state_count = 0;
    } else {
      unsigned int state_count;
      q->regions[i].state_sequence = (*extract_response_codes)(
          response_buf, response_bytes[i], &state_count);
...
u8 *choose_source_region(u32 *out_len) {
  ...
  *out_len = 0;
  ...
  if (q->region_count) {
    ...
    u32 len =
        q->regions[reg_index].end_byte - q->regions[reg_index].start_byte + 1;
    if (len <= MAX_FILE) {
      out = (u8 *)ck_alloc(len);
      if (out == NULL)
...
void update_fuzzs() {
  ...
  unsigned int *state_sequence =
      (*extract_response_codes)(response_buf, response_buf_size, &state_count);

  // A hash set is used so that the #paths is not updated more than once for one
  // specific state
  khash_t(hs32) * khs_state_ids;
  khint_t k;
  khs_state_ids = kh_init(hs32);
  ...
  for (i = 0; i < state_ids_count; i++) {
    ...
    if (k != kh_end(khms_states)) {
      ...
      case FAVOR:
        state->score =
            ceil(1000 *
                 pow(2, -log10(log10(state->fuzzs + 1) * state->selected_times +
                               1)) *
  ...
  if (k != kh_end(khms_states)) {
    ...
    case FAVOR:
      if (state->seeds_count > 10) {
        // Do seed selection similar to AFL + take into account state-aware
        // information e.g., was_fuzzed information becomes state-aware
        u32 passed_cycles = 0;
        while (passed_cycles < 5) {
          ...
          // Skip this seed with high probability if it is neither an initial
          // seed nor a seed generated while the current target_state_id was
          // targeted
          if (result->generating_state_id != target_state_id &&
              !result->is_initial_seed && UR(100) < 90)
            continue;

          u32 target_state_index = get_state_index(target_state_id);
          if (pending_favored) {
            /* If we have any favored, non-fuzzed new arrivals in the queue,
               possibly skip to them at the expense of already-fuzzed or
               non-favored cases. */
            if (((was_fuzzed_map[target_state_index][result->index] == 1) ||
                 !result->favored) &&
            ...
            /* Otherwise, still possibly skip non-favored cases, albeit less
               often. The odds of skipping stuff are higher for already-fuzzed
               inputs and lower for never-fuzzed entries. */
            if (queue_cycle > 1 &&
                (was_fuzzed_map[target_state_index][result->index] == 0)) {
    ...
    PFATAL("AFLNet - the states hashtable has no entries for state %d",
           target_state_id);
  }
...
void update_state_aware_variables(struct queue_entry *q, u8 dry_run) {
  ...
  if (is_state_sequence_interesting(state_sequence, state_count)) {
    // Save the current kl_messages to a file which can be used to replay the
    // newly discovered paths on the ipsm
    u8 *temp_str = state_sequence_to_string(state_sequence, state_count);
    u8 *fname = alloc_printf("%s/replayable-new-ipsm-paths/id:%s:%s", out_dir,
                             temp_str, dry_run ? basename(q->fname) : "new");
    ...
    if (state_count > 1) {
      ...
      for (i = 1; i < state_count; i++) {
        ...
        // Check if the prevStateID and curStateID have been added to the state
        // machine as vertices Check also if the edge prevStateID->curStateID
        // has been added
        Agnode_t *from, *to;
        Agedge_t *edge;
        ...
        if (!from) {
          ...
          state_info_t *newState_From =
              (state_info_t *)ck_alloc(sizeof(state_info_t));
          ...
          state_ids =
              (u32 *)ck_realloc(state_ids, (state_ids_count + 1) * sizeof(u32));
  ...
  // Update others no matter the new seed leads to interesting state sequence or
  // not

  // Annotate the regions
  update_region_annotations(q);

  // Update the states hashtable to keep the list of seeds which help us to
  // reach a specific state Iterate over the regions & their annotated state
  // (sub)sequences and update the hashtable accordingly All seed should "reach"
  // state 0 (initial state) so we add this one to the map first
  k = kh_get(hms, khms_states, 0);
  if (k != kh_end(khms_states)) {
    state = kh_val(khms_states, k);
    state->seeds = (void **)ck_realloc(state->seeds, (state->seeds_count + 1) *
                                                         sizeof(void *));
    ...
    PFATAL("AFLNet - the states hashtable should always contain an entry of "
           "the initial state");
  }
  ...
  for (i = 0; i < q->region_count; i++) {
    ...
    if (regional_state_count > 0) {
      ...
      unsigned int reachable_state_id =
          q->regions[i].state_sequence[regional_state_count - 1];

      k = kh_get(hms, khms_states, reachable_state_id);
      if (k != kh_end(khms_states)) {
        state = kh_val(khms_states, k);
        state->seeds = (void **)ck_realloc(
            state->seeds, (state->seeds_count + 1) * sizeof(void *));
        state->seeds[state->seeds_count] = (void *)q;
        state->seeds_count++;
      } else {
        // XXX. This branch is supposed to be not reachable
        // However, due to some undeterminism, new state could be seen during
        // regions' annotating process even though the state was not observed
        // before To completely fix this, we should fix all causes leading to
        // potential undeterminism For now, we just add the state into the
        // hashtable
      ...
      was_fuzzed_map[get_state_index(reachable_state_id)][q->index] =
          0; // Mark it as reachable but not fuzzed
    }
  }
...
int send_over_network() {
  ...
  // This piece of code is only used for targets that send responses to a
  // specific port number The Kamailio SIP server is an example. After running
  // this code, the intialized sockfd will be bound to the given local port
  if (local_port > 0) {
    ...
    if (bind(sockfd, (struct sockaddr *)&local_serv_addr,
  ...
  for (it = kl_begin(kl_messages); it != kl_end(kl_messages);
       it = kl_next(it)) {
    ...
    response_bytes =
        (u32 *)ck_realloc(response_bytes, messages_sent * sizeof(u32));
...
static void add_to_queue(u8 *fname, u32 len, u8 passed_det) {
  ...
  if (corpus_read_or_sync) {
    ...
    // We use this for some optimization to reduce the overhead while following
    // the server's sequence diagram
    ...
    q->regions = convert_kl_messages_to_regions(kl_messages, &q->region_count,
                                                messages_sent);
  }
  ...
  for (i = 0; i < MAP_SIZE; i++)
    if (top_rated[i] && (temp_v[i >> 3] & (1 << (i & 7)))) {
      ...
      if ((top_rated[i]->generating_state_id == target_state_id ||
          ...
          (was_fuzzed_map[get_state_index(target_state_id)]
                         [top_rated[i]->index] == 0))
        pending_favored++;
    }
...
static void read_testcases(void) {
  ...
  /* AFLNet: set this flag to enable request extractions while adding new seed
   * to the queue */
  ...
  /* AFLNet: unset this flag to disable request extractions while adding new
   * seed to the queue */
...
static void perform_dry_run(char **argv) {
  ...
  while (q) {
    ...
    /* Update state-aware variables (e.g., state machine, regions and their
     * annotations */
    if (state_aware_mode)
    ...
    u8 *fn_replay =
        alloc_printf("%s/replayable-queue/%s", out_dir, basename(q->fname));
...
static u8 save_if_interesting(char **argv, void *mem, u32 len, u8 fault) {
  ...
  if (fault == crash_mode) {
    ...
    /* We use the actual length of all messages (full_len), not the len of the
     * mutated message subsequence (len)*/
  ...
  case FAULT_TMOUT:
    ...
    fn = alloc_printf("%s/replayable-hangs/id:%06llu,%s", out_dir, unique_hangs,
                      describe_op(0));

#else

    fn = alloc_printf("%s/replayable-hangs/id_%06llu", out_dir, unique_hangs);
  ...
  keep_as_crash:
    ...
    fn = alloc_printf("%s/replayable-crashes/id_%06llu_%02u", out_dir,
...
static void maybe_delete_out_dir(void) {
  ...
  /* All right, let's do <out_dir>/replayable-crashes/id:* and
   * <out_dir>/replayable-hangs/id:*. */
...
#define SP20 SP10 SP10
  ...
  /* Show debugging stats for AFLNet only when AFLNET_DEBUG environment variable
   * is set */
  if (getenv("AFLNET_DEBUG") && (atoi(getenv("AFLNET_DEBUG")) == 1) &&
      state_aware_mode) {
    SAYF(cRST
         "\n\nMax_seed_region_count: %-4s, current_kl_messages_size: %-4s\n\n",
         DI(max_seed_region_count), DI(kl_messages->size));
    SAYF(cRST "State IDs and its #selected_times," cCYA "#fuzzs," cLRD
              "#discovered_paths," cGRA "#excersing_paths:\n");
    ...
    for (i = 0; i < state_ids_count; i++) {
      ...
      if (k != kh_end(khms_states)) {
        state = kh_val(khms_states, k);
        SAYF(cRST "S%-3s:%-4s," cCYA "%-5s," cLRD "%-5s," cGRA "%-5s",
             DI(state->id), DI(state->selected_times), DI(state->fuzzs),
             DI(state->paths_discovered), DI(state->paths));
        if ((i + 1) % 3 == 0)
          SAYF("\n");
      }
    }
...
EXP_ST u8 common_fuzz_stuff(char **argv, u8 *out_buf, u32 len) {
  ...
  if (!region_count){
    printf("Region Count inside common_fuzz_stuff %d\n",region_count);
  ...
  for (i = 0; i < region_count; i++) {
    ...
    *kl_pushp(lms, kl_messages) = m;
    ...
    // in fact, the old tail storage is used to store the newly added entry and
  ...
  // Update fuzz count, no matter whether the generated test is interesting or
  // not
  if (state_aware_mode)
    update_fuzzs();
...
AFLNET_REGIONS_SELECTION:;
  ...
  /* Identify the prefix M1, the candidate subsequence M2, and the suffix M3.
   * See AFLNet paper */
  /* In this implementation, we only need to indentify M2_start_region_ID which
  is the first region of M2 and M2_region_count which is the total number of
  regions in M2. How the information is identified is state aware dependent.
  However, once the information is clear, the code for fuzzing preparation is
  the same */
  ...
  if (state_aware_mode) {
    ...
    if (target_state_id == 0) {
      ...
      // To compute M2_region_count, we identify the first region which has a
      // different annotation Now we quickly compare the state count, we could
      // make it more fine grained by comparing the exact response codes
      for (i = 0; i < queue_cur->region_count; i++) {
        if (queue_cur->regions[i].state_count !=
            queue_cur->regions[0].state_count)
          break;
        M2_region_count++;
      }
    } else {
      ...
      for (i = 0; i < queue_cur->region_count; i++) {
        ...
        if (regionalStateCount > 0) {
          ...
          u32 reachableStateID =
              queue_cur->regions[i].state_sequence[regionalStateCount - 1];
          M2_start_region_ID++;
      ...
      for (i = M2_start_region_ID; i < queue_cur->region_count; i++) {
        if (queue_cur->regions[i].state_count !=
            queue_cur->regions[M2_start_region_ID].state_count)
          break;
        M2_region_count++;
      }
  ...
  /* Construct the kl_messages linked list and identify boundary pointers
   * (M2_prev and M2_next) */
  kl_messages = construct_kl_messages(queue_cur->fname, queue_cur->regions,
...
havoc_stage:
  ...
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
    ...
    for (i = 0; i < use_stacking; i++) {
      ...
      /* Values 17 to 20 can be selected only if region-level mutations are
       * enabled */
      ...
      /* Insert a random region from a random seed to the beginning of the
       * current region */
      case 18: {
        u32 src_region_len = 0;
      ...
      /* Insert a random region from a random seed to the end of the current
       * region */
      case 19: {
        u32 src_region_len = 0;
        u8 *src_region = choose_source_region(&src_region_len);
  ...
  while ((sd_ent = readdir(sd))) {
    ...
    while ((qd_ent = readdir(qd))) {
      ...
      if (st.st_size && st.st_size <= MAX_FILE) {
        ...
        /* AFLNet: set this flag to enable request extractions while adding new
         * seed to the queue */
        ...
        /* AFLNet: unset this flag to disable request extractions while adding
         * new seed to the queue */
        corpus_read_or_sync = 0;
...
static void usage(u8 *argv0) {
  ...
  SAYF(
      ...
      "DTLS12, DNS, SMTP, SSH, TLS)\n"
      "  -D usec       - waiting time (in micro seconds) for the server to "
      "initialize\n"
      "  -W msec       - waiting time (in miliseconds) for receiving the first "
      "response to each input sent\n"
      "  -w usec       - waiting time (in micro seconds) for receiving "
      "follow-up responses\n"
      "  -e netnsname  - run server in a different network namespace\n"
      "  -K            - send SIGTERM to gracefully terminate the server (see "
      "README.md)\n"
      "  -E            - enable state aware mode (see README.md)\n"
      "  -R            - enable region-level mutation operators (see "
      "README.md)\n"
      "  -F            - enable false negative reduction mode (see README.md)\n"
      "  -c cleanup    - name or full path to the server cleanup script (see "
      "README.md)\n"
      "  -q algo       - state selection algorithm (See aflnet.h for all "
      "available options)\n"
      "  -s algo       - seed selection algorithm (See aflnet.h for all "
      "available options)\n\n"
...
/* Check that afl-fuzz (file/process) has some effective and permitted
 * capability */
...
static int check_ep_capability(cap_value_t cap, const char *filename) {
  ...
  if (file_cap) {
    if (cap_get_flag(file_cap, cap, CAP_EFFECTIVE, &cap_flag_value))
      PFATAL("Could not get CAP_EFFECTIVE flag value from file \"%s\"",
             filename);
    ...
    if (cap_get_flag(file_cap, cap, CAP_PERMITTED, &cap_flag_value))
      PFATAL("Could not get CAP_PERMITTED flag value from file \"%s\"",
             filename);
...
int main(int argc, char **argv) {
  ...
  while ((opt = getopt(argc, argv,
                       "+i:o:f:m:t:T:dnCB:S:M:x:QN:D:W:w:e:P:KEq:s:RFc:l:")) >
         0)
    ...
    case 'P': /* protocol to be tested */
      ...
      } else if (!strcmp(optarg, "IEC61850")) {
  ...
  if (netns_name) {
    if (check_ep_capability(CAP_SYS_ADMIN, argv[0]) != 0)
      FATAL("Could not run the server under test in a \"%s\" network namespace "
            ...
            "afl-fuzz with sudo or by \"$ setcap cap_sys_admin+ep "
            "/path/to/afl-fuzz\".",
            netns_name);
  }
